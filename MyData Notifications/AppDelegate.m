...
NSString * const MyDataNotificationsBundleID = @"org.mytardis.mydata-notifications";
...
- (NSString *)__bundleIdentifier;
{
  if (self == [NSBundle mainBundle]) {
    return _fakeBundleIdentifier ? _fakeBundleIdentifier : MyDataNotificationsBundleID;
  } else {
...
+(void)initializeUserDefaults
{
  ...
  if (isMavericks()) {
    //10.9
    appDefaults = @{@"sender": @"org.mytardis.MyData"};
  } else {
    //10.8
    appDefaults = @{@"": @"message"};
  }
...
- (void)printHelpBanner;
{
  ...
  printf("%s (%s) is a command-line tool to send OS X User Notifications.\n" \
         ...
         "   Optional:\n" \
         "\n" \
         "       -title VALUE       The notification title. Defaults to ‘MyData’.\n" \
         ...
         "\n" \
         "For more information see https://github.com/wettenhj/mydata-notifications.\n",
...
- (void)applicationDidFinishLaunching:(NSNotification *)notification;
{
  ...
  if (userNotification) {
    ...
    if (message) {
      ...
      [self deliverNotificationWithTitle:defaults[@"title"] ?: @"MyData"
                                subtitle:subtitle
                                 message:message
                                 options:options
                                   sound:sound];
